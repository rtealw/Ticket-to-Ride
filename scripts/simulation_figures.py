...
def generateFigure(keys, freq2, freq4, title, filename, colors, xlabel, include_lines):
    ...
    directory = "{}/../paper/figures/".format(os.getcwd())
    plt.tight_layout()
...
def saveProportions(keys, props, filename):
    directory = "{}/input/".format(os.getcwd())
...
def readGamesAndGenerateFigures(filename, limit):
    ...
    tickets_keys = np.unique(
        list(two_tickets['win_count'].keys()) + list(two_tickets['lose_count'].keys()) + list(four_tickets['win_count'].keys()) + list(four_tickets['lose_count'].keys())
    )
    tickets_two = postProcessTickets(two_tickets, tickets_keys)
    tickets_four = postProcessTickets(four_tickets, tickets_keys)

    # Route Processing
    routes_keys = np.unique(list(routes['2-player'].keys()) + list(routes['4-player'].keys()))
